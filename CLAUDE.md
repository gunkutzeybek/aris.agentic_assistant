...
- **The cloud's `reachable: false` does not mean "broken".** `connector_summary()` derives it from `connector_last_ok_at`, and migration `0012` back-fills `mcp_url` + `connector_source='legacy'` for installer-era units **without** a probe timestamp — so every legacy home reports `configured: true, reachable: false` forever while working perfectly. Only a connector *we* registered has definitely been probed (`PUT /v1/units/connector` probes synchronously), so `coordinator.connector_reachable` returns `None` ("unknown") for anyone else's connector, and `home_unreachable` never fires on one. Hit for real on the dev box after the 0.5.0 upgrade — it raised a red error on a working home.
- **`home_not_connected` fires only when the cloud explicitly reports `configured: false`.** An absent `connector` block means an older cloud, not a disconnected home — the existing fleet is connected the legacy way (Homapel-run cfbridge tunnel + ha-mcp add-on, no connector keys in `entry.data`) and must not be nagged to reconfigure.
- **The fleet runs a different ha-mcp shape than this code targets** (checked on the dev box, 2026-08-23): the `ha_mcp_tools` component is absent; ha-mcp runs as the add-on pair `81f33d0f_ha_mcp` + `81f33d0f_ha_mcp_webhook_proxy` (its `mcp_proxy` integration serves the webhook, id in `hass.data["mcp_proxy"]["webhook_id"]`, config in `/config/.mcp_proxy_config.json`, same `ha_auth` bearer gate). **Decided 2026-08-24: not supported — component only.** Those homes keep their legacy cloud connector: `ConnectorManager` stays idle without connector keys, and reconfigure→skip only `DELETE`s when `connector_source` is set, so nothing tears them down. Migrating them means installing `ha_mcp_tools` and running Reconfigure.
